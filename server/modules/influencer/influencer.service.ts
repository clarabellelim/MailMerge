...
export class InfluencerService {
  ...
  private readonly memInfluencers = new Map<string, Influencer>();
  ...
  async getInfluencers(
    ...
    if (verified !== undefined) {
      conditions.push(eq(influencer.verified, verified));
    }
    ...
    const query = conditions.length > 0
      ? this.db.select().from(influencer).where(and(...conditions))
      : this.db.select().from(influencer);

    const items = await query
      .orderBy(desc(influencer.createdAt))
      .limit(pageSize)
      .offset(offset);
  ...
  async getInfluencersByIds(ids: string[]): Promise<Influencer[]> {
    ...
    const items = await this.db
      .select()
      .from(influencer)
      .where(inArray(influencer.id, ids));
  ...
  async updateInfluencer(
    ...
    if (!this.isDbAvailable()) {
      ...
      if (!existing) throw new BadRequestException('记录不存在');
    ...
    const updateData: Partial<typeof influencer.$inferInsert> = {};
  ...
  async uploadCsv(csvContent: string): Promise<InfluencerUploadResp> {
    const lines = csvContent.trim().split('\n');
    if (lines.length < 2) {
      throw new BadRequestException('CSV文件格式不正确');
    }
    ...
    if (handleIndex === -1 || platformIndex === -1) {
      throw new BadRequestException('CSV文件缺少必要的列');
    }
  ...
  async uploadFile(file: any): Promise<InfluencerUploadResp> {
    ...
    if (name.endsWith('.xlsx') || name.endsWith('.xls')) {
      ...
      if (!sheetName) throw new BadRequestException('Excel文件中没有工作表');
      const ws = wb.Sheets[sheetName];
      const rows = XLSX.utils.sheet_to_json<Record<string, any>>(ws, { defval: '' });
      return this.uploadRows(rows);
    }

    throw new BadRequestException('仅支持 .csv / .xlsx 文件');
  }

  private async uploadRows(rows: Record<string, any>[]): Promise<InfluencerUploadResp> {
  ...
  private mapToInfluencer(item: typeof influencer.$inferSelect): Influencer {
    return {
      id: item.id,
      handle: item.handle,
      ...
      scanStatus: item.scanStatus as 'complete' | 'partial' | 'failed',
